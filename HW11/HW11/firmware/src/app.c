        ...
        case APP_STATE_SCHEDULE_WRITE:
            ...
            while(counter<100){
                ...
                if (counter==0){
                    ...
                    IIR = (IIR*A) + ((gyro_z)*(1-A));
                }
                k = 0;
                for (k=(buff_size-2); k>-1; k--){
                    MAF_buff[k+1] = MAF_buff[k]; 
                }
                MAF_buff[0] = gyro_z;
                MAF = 0;
                FIR = 0;
                for (k=0;k<buff_size;k++){
                    MAF = MAF + MAF_buff[k];
                    FIR = FIR + MAF_buff[k]*FIR_weights[k];
                }
                MAF = MAF/(buff_size);
                ...
                //len = sprintf(dataOut, "%d: %d, %d, %d, %d, %d: %d, %d, %d, %d\r\n", counter, MAF_buff[0], MAF_buff[1], MAF_buff[2],MAF_buff[3],MAF_buff[4],MAF_buff[5],MAF_buff[6],MAF_buff[7]);
                len = sprintf(dataOut, "%d: %d, %f, %f, %f\r\n", counter, gyro_z, MAF, IIR, FIR);
                

                i++; // increment the index so we see a change in the text
                /* IF A LETTER WAS RECEIVED, ECHO IT BACK SO THE USER CAN SEE IT */
                if (appData.isReadComplete && (flag_r==1)) {
